...
$conn = $pdo->open();
$stmt = $conn->query("SELECT details.supplier as supplier ,count(*) as number FROM details GROUP BY details.supplier");
$stmt1 = $conn->query("SELECT details.company_name as retailer,count(*) as number2 FROM products LEFT JOIN details ON products.id=details.product_id JOIN users ON details.supplier=users.company_name GROUP BY details.company_name");
